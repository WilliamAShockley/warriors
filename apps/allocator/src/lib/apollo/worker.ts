...
STEP 3 — Stage it VERBATIM. stage_proof with kind "email", the skill's subject and body EXACTLY as returned — do not polish, shorten, or fix its grammar; its quirks are the reader's own voice — plus the recipient's address in "to", the threadId when it is a follow-up, todoId "${todoId}" so signing it clears the to-do, and grounding = the same context you handed the skill (thread summary or research findings, with source attributions). The reader highlights lines in the proof and asks where they came from; the grounding is what answers him. You must ALWAYS stage exactly one proof, even if research came up thin — a flagged best-guess beats no draft.
